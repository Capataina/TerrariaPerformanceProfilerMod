...
- [ ] The `insights` LiteDB collection, its row, its op kind, and its stream all exist, but **nothing enqueues a `DbWriteOp.Insight`** — the live feed is in-memory only. — DEFERRED (not-free, recommended-default): the cleanup target (`Profiling/Persistence/DbWriteOp.cs`, `Data/Streams/InsightStream.cs`, `ProfilerDatabase.cs`) is outside the `Insights/`-only edit scope of this implementation pass; the producer itself is feature work. Leave for the feature work; the system doc already records the gap.
...
- [x] `Reset()` exists with a docstring claiming an "end-of-session pass to drain every spike" uses it, but no call site exists anywhere in the tree. — IMPLEMENTED: deleted the dead `Reset()` method + misleading docstring from `PeakContributorToSpikeDetector.cs` (behaviour-identical, never invoked).
...
- [x] The reverse-Chan M2 recovery subtracts two nearly-equal large doubles; when the in-context and out-of-context distributions are similar, floating-point cancellation can drive the recovered M2 negative, and the code floors it to 0 — producing a degenerate (zero-variance) complement that the downstream Welch test reads as "infinitely confident". — IMPLEMENTED: replaced the bare `if (m2 < 0d) m2 = 0d` floor in `RunningStat.Without` with a relative-epsilon cancellation guard (`m2 < WithoutRelEps * _m2`, `WithoutRelEps = 1e-9`) that falls back to the parent's per-sample spread for the complement instead of a degenerate zero variance; well-conditioned recoveries are untouched (exact-recovery regression verified). Diagnostic test `Tests/AuditPin_Insights_Without.cs` exercises the symmetric + asymmetric cancellation regimes and the well-separated regression, asserting non-negative non-degenerate variance and that Welch does not report spurious significance. NOTE: a *deeper* spurious-significance path (recovered MEAN drift at very large magnitudes, e.g. mean ≈ 1e8, that no defensible variance floor fixes without corrupting the well-conditioned path) was discovered during probing and is DEFERRED — it needs a `RunningStat` trustworthiness flag or a `WelchTTestP` no-evidence contract change, both broader than this finding's scoped guard and the `Insights/`-only edit surface.
...
- [x] `WelchTTestP` normal-approximates the t-distribution and never applies the Welch–Satterthwaite degrees-of-freedom correction; the per-mod cost series is right-skewed (sub-ms with occasional spikes) and the in/out sample sizes are unequal — the exact regime the literature shows inflates the false-positive rate. — IMPLEMENTED (free sub-part only): corrected the overclaiming `WelchTTestP` docstring — "accurate" now reads "approximate; on right-skewed low-mean cost with unequal n the normal approximation can be slightly anti-conservative, partially offset by the detectors' Bonferroni step", and names the omitted Welch–Satterthwaite df. The t/df or Mann–Whitney fix is behaviour-changing feature work and remains DEFERRED.
...
- [x] Each detector corrects by `testsRun` (the count of comparisons that pass the candidate gate this pass), but those tests are NOT independent — every bucket's out-of-context complement is derived from the same global per-mod series — so the effective number of independent tests is smaller than `testsRun`. — IMPLEMENTED: added the conservative-by-dependence comment at all four Bonferroni-denominator sites (`ContextConditionalCostDetector`, `SustainedCostShiftDetector`, `ContextCorrelatedSpikeDetector`, `NewContributorDetector`), each noting the shared baseline makes the correction suppress real signals (never manufacture them) and that a Holm/BH step would recover the power. Comment-only.
...
- [x] When the early window mean is ~0, `RatioOrDelta` is set to a hardcoded `99d` ("~from-zero"); this is a ratio pattern, so `RankingScorer.RatioCurve` saturates it to 1.0 — every new-contributor record pins maximum magnitude regardless of how small the late cost actually is. — IMPLEMENTED (free sub-part only): replaced the terse `// ~from-zero` comment on `NewContributorDetector.cs:74` with a full note recording the ranking consequence (sentinel saturates magnitude to 1.0; player copy renders `late` ms so the surface stays honest; the distortion is ranking-order only) and that ranking on a `MagnitudeShape.Rate` late level would be more honest. The magnitude fix itself is behaviour-changing (alters ranking order) and remains DEFERRED.
...
- [x] `RankingScorer.NormaliseMagnitude`'s docstring enumerates the share patterns as "`HotHookDominance`, `AllocationBurst`, `PeakContributorToSpike`" but `IsSharePattern` actually returns true for 7 patterns (those three plus `GcPauseCulprit`, `CostConcentration`, `FrameHeadroom`, `FrameJitter`). — IMPLEMENTED: the `NormaliseMagnitude` docstring `<list>` now names all seven share patterns and points to `IsSharePattern` as the authority. Comment-only.
...
- [x] `ContextCorrelatedSpike` stores `lift = spikeShare/dwellShare` (a ratio, can be ≫1) in `RatioOrDelta`, is correctly excluded from `IsSharePattern`, and renders as "1.8×" — but the system doc's "structural-fact patterns report shares against an explicit ceiling" framing reads as if it were a share pattern. Minor classification ambiguity, not a bug. — IMPLEMENTED (free sub-part — code comment, in scope): strengthened the inline comment at `ContextCorrelatedSpikeDetector.cs:90` to state the value is an over-representation LIFT ratio (can be ≫1, not a [0,1] share), correctly excluded from `IsSharePattern`, and must not be moved into the share set (which would clamp a legitimate 3× lift). The system-doc note for the next `upkeep-context` pass is outside this repo-edit scope.
...
- [ ] `ContextConditionalCost`, `SustainedCostShift`, `NewContributor` (and structurally `ContextCorrelatedSpike`) repeat the same five-step body: clear scratch, sweep + count `testsRun`, candidate-gate (`mean` direction + Cohen's d ≥ 0.8), `adjust = max(1, testsRun)`, sort-by-effect, emit-top-K with `pAdjusted = min(1, p*adjust)`. — DEFERRED (not-free, recommended-default): the finding itself marks this NOT a free win — a naive `Func<>`-based `TwoSampleSweep` extraction risks per-candidate closure allocations on the off-thread path (Invariant 2). An allocation-free struct-predicate extraction is a real refactor, not a pass-through cleanup; left as a watch-item.
...
- [ ] `ModObservatoryStat`, `EngagementCostScatterStat`, `DormantSurfaceStat` each open with the same `DataRegistry.Shared.Lookup<X>(name)?.CurrentSnapshot() ?? X.Empty` triple-lookup and the same "build a `Dictionary<int, ModUsageEntry>` keyed by ModId then accumulate `UsageWeight`" loop. — DEFERRED (not-free, recommended-default): the finding marks this borderline-free and "surface before editing rather than treating as a pass-through cleanup" — it restructures three live `Publish/` stats. Recommended-default per the audit brief = do not make the structural change; left for a dedicated refactor pass.
...
- [ ] The variance pre-pass (`:147-160`) and the `Pearson` covariance loop (`:202-219`) each re-walk every bucket subtracting the mean; for the included N×N upper triangle this re-reads `series2d` `O(N² · bucketCount)` times with the mean-subtraction inlined, when the deviations could be cached once. — DEFERRED (not-free, recommended-default): the finding explicitly recommends NO change — the cached-deviation-matrix alternative trades a free ~864 KB allocation for a saving nobody can feel behind the 5 s recompute gate, against Invariant-2 memory discipline. Recorded so the "optimise Pearson" impulse sees the trade was weighed and rejected.
...
- [ ] Each `Evaluate` allocates `new CollectorInsightInput(collector)` to feed the temporal baseline; at 1 Hz off-thread this is negligible, but it is a small per-pass heap allocation in a profiler whose entire thesis is measuring allocations. — DEFERRED (not-free, recommended-default): the finding marks the field-cache fix "Free, but a touch to the hot-ish path — surface before editing", and the edit target `InsightsEngine.cs:281` sits in the off-thread eval loop the brief flags as allocation-sensitive. Recommended-default = surface before touching the eval path; left for explicit confirmation rather than folded into this pass.
