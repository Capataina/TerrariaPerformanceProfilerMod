...
- **Insights consolidated into a top-level `Insights/` module (v0.18.1 → v0.19.0, the eight-wave rework).** The interpretation layer was scattered across `Data/Detectors/Insights/`, `Data/Stats/`, `Data/Aggregators/`, and inline router code, with the same formulas duplicated up to eight times. It was lifted to a peer top-level `Insights/` module — the same single-home / single-export-seam discipline v0.10–v0.12 did for *data*, one level up. Decision: **not** a flat mirror of the data pipeline — insights earned richer internal modularisation (ReferenceFrames, Drivers, Detectors-by-family, Shared primitives, Publish stats) because interpretation is a harder workflow than capture. The spine law generalised here ("relative to a reference frame, never absolute, as an effect size") is now encoded verbatim in `IReferenceFrame.cs`. Roster grew from 4-live/6-gated to **16 detectors (13 live, 3 gated) across five families** on Welford-online baselines with Welch/Cohen's-d/Bonferroni guards and fingerprint-keyed cross-session persistence (`contextBaselines`). Two store bugs closed: G5 (a `_topComparerNowTick` race → a `_topGate` lock) and G6 (a 64-bit packed dedup key → a full-width `InsightKey`). **The Flute fix (Wave 4):** the usage axis measured content *created* (`OnCreated` fires only on craft/buy/init/journey-dup), so a held-but-not-crafted weapon read 0 usage and the whole engagement share collapsed; rewired to active-use ticks (`ItemsHeldTicks`/`ArmorEquippedTicks`). Three detectors stay gated on named prerequisites: `HookFrequencyTail` (needs per-call hook histograms — an unmeasured hot-path addition, blocked by Invariant 2), `LoadoutCombinationCost` (cross-session loadout aggregation), cross-mod event chains (research-gated to avoid manufacturing spurious patterns, Invariant 3).

- **Dashboard rebuilt on a shadcn-neutral OKLCH component library (v0.17).** Each pane was coded individually, so a class of bugs (leaking strips, scroll regions that did not scroll, mis-centred placeholders, inconsistent hover/selection, per-pane chart axes) recurred per pane. Decision: build a small set of canonical components (Panel, ScrollRegion, DataTable, SplitBar, RowList, Drawer, KpiTile + a chart layer) over a shadcn-rooted OKLCH token system, so spacing/scroll/selection/charting live in one place and the only thing varying per surface is content. House rule: **monochrome chrome, colourful data** — colour only on the data-viz layer, always encoding. CSS bundle shrank ~84 KB → ~55 KB.

- **A richer chart vocabulary + the sixth (Memory) tab (v0.20 → v0.22).** Four new encodings landed in the shared `Js.Charts` module — bubble scatter (engagement-vs-cost), sankey (category→mod cost-flow), waffle (modlist composition), and a per-mod cost-stream (stacked bands over a rolling window). The cost-stream's band ramp was switched from a red→green perf ramp to a pure monochrome luminance ramp because the hued version "did not read as nice" and collided with the monochrome-chrome rule; magnitude now reads as luminance + thickness. A sixth **Memory** tab was added for per-mod RAM (profiler scaffolding distributed by hook-share + tModLoader's own estimate), the one router arm that joins a non-`DataRegistry` source (`ModRamReader`, guarded reflection into tML internals, honesty-degrading to `tmlAvailable:false`).

- **An off-game L4/L6/L8 testing harness (`tools/testing/`).** The dashboard is the product's surface but only one human could judge it, capping every UI iteration at human-review speed. Decision: a self-describing harness — L4 (deterministic Playwright layout/interaction invariants), L6 (generative fixtures from the discovered `/api` contract), L8 (an agent-driven vision audit that fans out per tab and writes durable `context/pages/` dossiers). Genericised so a new tab is audited with zero harness change. The first audit found 53 findings (6 P1); a fix-wave converged it to ~0 substantive. A recurring lesson the harness taught: **DOM/computed-style probes beat vision screenshots** for tabular structure and fine contrast (~4 vision false-positives were overturned by direct probes). L2 (hot-path budget), L5 (visual regression), L7 (in-game) remain unbuilt.

- **RAM trim.** The profiler's own footprint was cut ~3.7 GB → ~1.0 GB on a kitchen-sink stack (≈58 → 30 KB/hook) via a forced Gen2 at install-end + `TrimRetainedScaffolding` (guarded reflection releasing MonoMod/Cecil per-hook `ILHookEntry.LastContext`), coverage intact (62,203 detours = 62,203 contexts disposed).

**Honest caveats carried forward:** almost the entire arc is **RUNTIME-UNVERIFIED in-game** (the `.tmod` loader-lock blocks a packaged dev run; verified off-game via the preview render + L4 asserts + `dotnet test`). The **player-facing insight feed is not rendered yet** — the engine ranks insights but the Insights tab shows the observatory + charts, not the feed (the named next item). The **per-insight LiteDB `insights` collection is scaffolded but has no producer** — the live feed reaches the dashboard purely in-memory; the cross-session persistence that *is* live is the reference-frame `contextBaselines`, not per-insight rows.
